...
describe("replicate backup", () => {
	test("writes a dated, parseable snapshot and returns its summary", async () => {
		...
		expect(basename(result.file)).toBe("data.20260810-041723Z.snapshot.json");
	...
	test("the injectable clock controls both filename and takenAt", async () => {
		...
		expect(basename(result.file)).toBe("data.20311203-010203Z.snapshot.json");
	...
	test("snapshot failure leaves no new or temporary file and does not prune", async () => {
		const ctx = await fixture();
		const old = join(ctx.dir, "data.20200101-000000Z.snapshot.json");
	...
	test("retention prunes only strict matches for this project", async () => {
		...
		for (const name of [
			...
			"data.20210101-000000Z.snapshot.json",
			"data.20220101-000000Z.snapshot.json",
			"other.20200101-000000Z.snapshot.json",
			"data.snapshot.json",
			"data.20200101-000000Z.snapshot.json.tmp-1",
	...
	test("escapes regex metacharacters in an otherwise safe identifier", async () => {
		...
		const exact = "d.a.ta.20200101-000000Z.snapshot.json";
		const unescapedInterpretation = "dxayta.20200101-000000Z.snapshot.json";
	...
	test("retention attempts every deletion and aggregates failures", async () => {
		const ctx = await fixture();
		const undeletable = "data.20200101-000000Z.snapshot.json";
		const deletable = "data.20210101-000000Z.snapshot.json";
	...
	test("concurrent same-second publication atomically preserves one backup", async () => {
		...
		const file = join(ctx.dir, "data.20260810-041723Z.snapshot.json");
